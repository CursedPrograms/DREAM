"""
dream_memory.py - DREAM's long-term memory, shared by app.py (web) and
dream.py (voice) so both read and write the same files.
...
The most relevant facts are added to the model's prompt, each with how long
ago it was learned, via memory_prompt_block().
"""
...
MEMORIES_PATH = os.path.join(_ROOT, "memories", "memories.txt")
MILESTONES_PATH = os.path.join(_ROOT, "memories", "mymilestones.txt")
...
def recent_memories(limit=MAX_MEMORIES_IN_PROMPT):
    ...
    kept = {}
    for when, kind, text in _read_memories():
