...
QUESTIONS = {
    ...
    "home": ["Where in the world do you live? I like knowing where my human is.",
             "Where are you based? I've only ever seen this one room."],
...
INNATE = [
    ...
    {"id": "watch_room", "text": "Keep an eye on the room", "kind": "maintain", "priority": 0.5},
...
class Executive:
    ...
    def _allowed_to_speak(self, p, cand):
        ...
        urgent = cand["kind"] in ("reminder", "alert")
    ...
    def candidates(self, p):
        ...
        if p["motion_unspoken"] and d["security"] > 0.5:
            add("alert", 0.9, "there was motion and I'm on edge", "I saw something move in the room a moment ago.",
    ...
    def decide(self, p):
        ...
        if not options:
            return None
        forced = [c for c in options if c["kind"] in ("reminder", "alert")]
    ...
    def act(self, cand, now=None):
        ...
        if cand["kind"] not in ("reminder", "alert"):
    ...
    def _after(self, cand, now):
        ...
        elif what == "mark_dream":
            m.dreamer.mark_shared(arg)
