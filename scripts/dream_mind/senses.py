"""
senses.py - when she is, and what her body (this computer) feels like.
  ...
  Body       interoception: the machine she runs on, read through psutil. A busy
             ...
             like running low. How long the computer has been on, and how long
             she has been awake.
...
def known_birthday():
    ...
    try:
        from dream_memory import _read_memories
        for _, kind, text in reversed(_read_memories()):
            if kind == "birthday":
...
class Body:
    ...
    def report(self, now=None):
        ...
        r = self.read(now)
        if not r:
            return "I can't feel my body right now. The computer isn't telling me anything."
        bits = []
        ...
        up = r.get("boot_ts")
        tail = f" This computer has been on for {duration_words((now or time.time()) - up)}." if up else ""
        said = ", ".join(bits)
