...
variation, and the pause between sentences - so a sleepy DREAM drawls and a
...
def piper_args(prosody):
    """The prosody dict as command-line flags for piper."""
    return [
        "--length-scale", str(prosody["length_scale"]),
        "--noise-scale", str(prosody["noise_scale"]),
        "--noise-w-scale", str(prosody["noise_w_scale"]),
        "--sentence-silence", str(prosody["sentence_silence"]),
    ]
