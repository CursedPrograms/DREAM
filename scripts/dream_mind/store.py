...
ROOT = Path(__file__).resolve().parents[2]  # repo root (scripts/dream_mind/store.py)
_dir = ROOT / "memories" / "mind"
LEGACY_DIR = ROOT / "memories"   # mymilestones.txt, dreams.txt: the human-readable files
IMAGES_DIR = ROOT / "output" / "dreams"   # painted dreams (gitignored)
...
def set_dir(path):
    ...
    global _dir, LEGACY_DIR, IMAGES_DIR
    _dir = Path(path)
