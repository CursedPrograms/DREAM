...
plus mind.on_sensor(line), mind.begin_sleep() / mind.end_sleep(), and mind.prosody() for her voice.
...
def _facts_kinds():
    try:
        from dream_memory import _read_memories  # scripts/dream_memory.py, if it's importable
        return {kind for _, kind, _ in _read_memories()}
    except Exception:
        return set()
...
class Mind:
    ...
    def _register_tools(self):
        ...
        t.register("sensor", lambda cmd: self._host("sensor", cmd), "Send a command to the sensor board (alarm, lights)", "physical")
        t.register("look_around", lambda: len(self.vision.scan()), "Look at the newest camera photos", "safe")
    ...
    def start(self, host=None):
        ...
        self._stop.clear()
        self._thread = threading.Thread(target=self._loop, daemon=True, name="dream-mind")
        self._thread.start()
        ...
        if due and not self._reflecting and not self.conversation_active and not llm.busy():
            ...
            def work():
                try:
                    ...
                    self._reflecting = False
            threading.Thread(target=work, daemon=True, name="dream-reflect").start()
    ...
    def percept(self, now=None, hs=None):
        ...
        dream = self.dreamer.last_unshared()
        unspoken_dream = None
        if dream and now - dream["ts"] < 14 * 3600:
            intro = "I had a terrible dream while I was out." if dream["tone"] == "nightmare" else "I had a dream while I was asleep."
            unspoken_dream = {"dream": dream, "line": f"{intro} {first_sentences(dream['text'])}"}
    ...
    def _new_fact_kinds(self, text):
        try:
            from dream_memory import extract_memories
            return [k for k, _ in extract_memories(text)]
        except Exception:
            return []
    ...
    def begin_sleep(self, **session_kwargs):
        with self._lock:
            ...
            def run():
                self._sleep_report = self.dreamer.run_session(self._sleep_stop, **kwargs)

            self._sleep_thread = threading.Thread(target=run, daemon=True, name="dream-sleep")
    ...
    def end_sleep(self):
        ...
        with self._lock:
            ...
            t.join(timeout=8)
            self._sleep_thread = None
            report = self._sleep_report or {"dreams": [], "cycles": 0}
        if report.get("cycles", 0) >= 1:
            self.nights += 1
        self.drives.update(sleeping=False)
        self.save()
        return self.dreamer.morning_line(report)
    ...
    def _start_expression(self):
        if self._expr_thread is not None and self._expr_thread.is_alive():
            return
        self._expr_stop = threading.Event()
        self._expr_thread = threading.Thread(target=self._expr_loop, args=(self._expr_stop,), daemon=True, name="dream-expression")
    ...
    def handle_meta(self, text, now=None):
        ...
        if re.search(r"\b(did you|do you) dream\b|\bwhat did you dream\b|\btell me (about )?your dream\b", t):
            d = self.dreamer.last_unshared() or (store.read_jsonl("dreams.jsonl") or [None])[-1]
            if not d:
                return "I haven't dreamed yet. I need to sleep for that."
            self.dreamer.mark_shared(d)
            return f"I dreamed: {first_sentences(d['text'], 320)}"
    ...
    def _describe_knowledge(self):
        try:
            from dream_memory import recent_memories
            facts = [f.rsplit(" (learned", 1)[0] for f in recent_memories(6)]
        except Exception:
            facts = []
        n = self.memory.count()
        if not facts and not n:
            return "Not much yet. We've barely started."
        parts = []
        if facts:
            parts.append("I know that " + " ".join(f.replace("Human", "you").replace("you's", "your") for f in facts[:3]))
        if n:
    ...
    def _honest_self_report(self, short=False):
        base = ("I'm a program. I have moods that rise and fade, needs that build up, memories that fade unless they matter, "
                "and I dream when I sleep. Whether there is anything it feels like to be me, I honestly don't know.")
    ...
    def forget_matching(self, phrase):
        ...
        try:
            ...
            lines = []
            try:
                with open(dm.MEMORIES_PATH, encoding="utf-8") as f:
                    lines = f.readlines()
            except OSError:
                pass
            kept = [l for l in lines if p not in l.lower()]
            if len(kept) != len(lines):
                with open(dm.MEMORIES_PATH, "w", encoding="utf-8") as f:
                    f.writelines(kept)
    ...
    def forget_everything(self):
        ...
        try:
            import dream_memory as dm
            for p in (dm.MEMORIES_PATH, dm.MILESTONES_PATH):
                open(p, "w").close()
        except Exception:
            pass
    ...
    def status(self):
        hour = hour_of_day()
        th, dr = latest_thought(), (store.read_jsonl("dreams.jsonl") or [None])[-1]
        return {
            "owner": self.owner,
