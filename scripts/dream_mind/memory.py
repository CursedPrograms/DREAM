...
class Memory:
    ...
    def fragments(self, n=3, prefer_emotional=True, now=None):
        """Memories to dream about: weighted toward the salient and the emotional
        (and the unresolved - sad ones), with chance mixed in."""
        now = now or time.time()
        pool = self.active()
        if not pool:
            return []
        weights = []
        for e in pool:
            w = 0.1 + e["strength"] + (0.6 * e["intensity"] if prefer_emotional else 0.0)
            if e["valence"] < -0.2:
                w *= 1.4
            weights.append(w)
        picks, seen = [], set()
        for _ in range(min(n * 3, len(pool) * 2)):
